...
int main(int argc, char** argv) {
    ...
    for (int p = 0; p < chain; p++) {
        ...
        for (int l = 0; l < iter; l++) {
            #pragma omp target teams distribute map(from:C[0:d1*d3]) map(to:A[0:d1*d2]) map(to:B[0:d2*d3]) device(TARGETDART_ANY) nowait depend(inout:pipe[i]) depend(in:chain)
            for (int i = 0; i < d1; i++) {
