...
int main(int argc, char** argv) {
    ...
    for (int l = 0; l < iter; l++) {
        #pragma omp target teams distribute parallel for map(from:C[0:d1*d3]) map(to:A[0:d1*d2]) map(to:B[0:d2*d3]) map(to:d1,d2,d3) device(l%(omp_get_num_devices() + 1)) collapse(2) nowait
        for (int i = 0; i < d1; i++) {
            for (int k = 0; k < d3; k++) {
                C[i * d3 + k] = 0;
                for (int j = 0; j < d2; j++) {                
